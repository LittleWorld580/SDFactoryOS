...
#######################################
# EASYROMS UPDATE SCRIPT
# Pull EASYROMS files from GitHub repo and replace matching local files
# Does NOT delete extra local files
#######################################

# ===== CONFIG =====
REPO_URL="https://github.com/LittleWorld580/EASYROMS.git"
BRANCH="main"

# Where the EASYROMS files should be copied to
TARGET_DIR="/home/sdfactory/EASYROMsource/EASYROMS"
...
# Temp working folder
TMP_DIR="/tmp/easyromsupdate_repo"
...
fail() {
    echo "[EASYROMSUPDATE] ERROR: $*" >&2
    exit 1
}

require_root() {
    if [ "${EUID:-0}" -ne 0 ]; then
        fail "Run with sudo: sudo bash easyromsupdate.sh"
    fi
}
...
copy_files() {
    log "Copying EASYROMS files into $TARGET_DIR ..."
    rsync -av \
        "${RSYNC_EXCLUDES[@]}" \
